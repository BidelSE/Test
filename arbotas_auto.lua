...
--               Multiple empty rows may appear as decoys – ignored.
--               The header line ("Pasirinkite eilutę su visais 3 baltais
--               skaičiais") also uses white-ish codes, so the white check
--               REQUIRES the row text to match "N N N" before accepting it.
--
--   "tuscia"  – exactly one row is blank (space that stripColor trims to "").
--               Only reached when no white-numbers row exists.
--
...
local function whiteCodeCount(rawLine)
    local count = 0
...
-- Strict guard: the row must read as exactly three whitespace-separated
-- numbers, otherwise the header line ("...3 baltais skaičiais") would match
-- because it also uses white color codes.
...
local function scanArbotasDialog()
    ...
    -- "baltai" check first. The row must (a) carry 3+ white-ish color codes
    -- AND (b) read as "N N N" – this skips the header row that also uses
    -- white codes around its non-numeric text.
    for i, rawLine in ipairs(rawLines) do
        if whiteCodeCount(rawLine) >= 3 and looksLikeThreeNumbers(stripped[i]) then
            return dialogId, i - 1, "baltai"
        end
    end

    -- "tuscia" check: exactly one blank row, no white-numbers row present.
